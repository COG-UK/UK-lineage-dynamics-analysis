...
Due to ethical and legal restrictions we cannot upload =IATA_CountryLevel_Dec_May.csv=. To download the
...
France where various provinces are not included in the primary French death
toll. 
...
numbers, on these days we set the daily value to zero. Since subtracting
